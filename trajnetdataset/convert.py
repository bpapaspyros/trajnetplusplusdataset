...
def write(input_rows, output_file, args):
    ...
    if args.order_frames:
        ...
        frames = sorted(
            set(input_rows.map(lambda r: r.frame).toLocalIterator()))
    ...
    train_scenes = Scenes(fps=args.fps, start_scene_id=0,
                          args=args).rows_to_file(train_rows, train_output)

    # validation dataset
    val_rows = input_rows.filter(lambda r: r.frame in val_frames)
    val_output = output_file.format(split='val')
    val_scenes = Scenes(fps=args.fps, start_scene_id=train_scenes.scene_id,
                        args=args).rows_to_file(val_rows, val_output)

    # public test dataset
    test_rows = input_rows.filter(lambda r: r.frame in test_frames)
    test_output = output_file.format(split='test')
    test_scenes = Scenes(
        fps=args.fps, start_scene_id=val_scenes.scene_id, args=args)  # !!! Chunk Stride
    test_scenes.rows_to_file(test_rows, test_output)

    # private test dataset
    private_test_output = output_file.format(split='test_private')
    private_test_scenes = Scenes(
        fps=args.fps, start_scene_id=val_scenes.scene_id, args=args)
...
def categorize(sc, input_file, args):
    ...
    if args.train_fraction:
        ...
        train_rows = get_trackrows(
            sc, input_file.replace('split', '').format('train'))
    ...
    if args.val_fraction:
        ...
        val_rows = get_trackrows(
            sc, input_file.replace('split', '').format('val'))
    ...
    if test_fraction:
        ...
        test_rows = get_trackrows(sc, input_file.replace(
            'split', '').format('test_private'))
...
def edit_goal_file(old_filename, new_filename):
    ...
    shutil.copy("goal_files/train/" + old_filename,
                "goal_files/train/" + new_filename)
    shutil.copy("goal_files/val/" + old_filename,
                "goal_files/val/" + new_filename)
    shutil.copy("goal_files/test_private/" + old_filename,
                "goal_files/test_private/" + new_filename)
...
def main():
    ...
    parser.add_argument('--fish_data_path', type=str, default='',
                        help='Path to fish data (generated or raw)')
    ...
    # Synthetic datasets conversion
        ...
        edit_goal_file('orca_circle_crossing_5ped_1000scenes_.pkl',
                       'orca_five_synth.pkl')
